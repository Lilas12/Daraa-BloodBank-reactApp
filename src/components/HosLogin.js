...
function Login({ onLogin }) {
  ...
  const onSubmit = async (data) => {
    ...
    setError("");
    try {
      // Sätter session så man slipper logga in igen om man uppdaterar sidan (valfritt)
      await setPersistence(auth, browserSessionPersistence);
      ...
      // Skicka användaren vidare till huvudsystemet
      onLogin(cred.user);
    } catch (error) {
    ...
    <Container>
      ...
      <AnimatePresence>
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            {error && <StatusBox>{error}</StatusBox>}
